          ...
          {!isLoading &&
            filtered.map((app) => (
              <button
                key={app.exec_path}
                onClick={() => onPick(app)}
                className="flex w-full items-center gap-2.5 rounded-md border border-border bg-base px-3 py-2 text-left text-xs hover:border-accent/50 hover:bg-overlay/[0.03]"
              >
